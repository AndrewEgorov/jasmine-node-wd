module.exports = function (cwd, env, argv, completeCallback) {

    if (typeof cwd == "string") {
        var cwds = cwd;
        cwd = function() {  return cwds; };
    }
...
// Disable if we're not on a TTY
    ...
    var teamcity = env.TEAMCITY_PROJECT_NAME || false;
    ...
    while (args.length) {
        ...
        default:
            ...
            if (arg.match(/^\/.*/)) {
                ...
                specFolders.push(Path.join(cwd(), arg));
            }
            break;
        }
    }
    ...
    function onExit() {
        console.log('On cli-module exit');
